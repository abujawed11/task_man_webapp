  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-300 via-yellow-100 to-white py-10 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {notifications.length === 0 || notifications.filter(n => !hiddenNotificationIds.includes(n.notification_id)).length === 0 ? (
            <div className="text-center text-black text-lg col-span-full">
              <p>No unread notifications.</p>
            </div>
          ) : (
            notifications
              .filter((note) => !hiddenNotificationIds.includes(note.notification_id))
              .map((note, index) => (
                <div
                  key={index}
                  // className="bg-white border border-yellow-500 rounded-lg shadow-lg hover:shadow-xl transition duration-300"
                ...
                >
                  ...
                  <div className="p-4 text-black text-sm space-y-2">
                    ...
                    {note.type === 'task_created' && (
                      <p>
                        📌 You have been assigned a new task: "<strong>{note.message}</strong>" by <strong>{note.sender}</strong>.
                      </p>
                    )}

                    {note.type === 'task_updated_by_creator' && note.updates && (
                      <>
                        <p>✏️ The task "<strong>{note.task_title}</strong>" has been updated by <strong>{note.sender}</strong>.</p>
                        <p>
                          ➡️ Changes:{" "}
                    ...
                    {note.type === 'task_updated' && note.updates && (
                      <>
                        <p>✅ Your assigned task "<strong>{note.task_title}</strong>" has been updated by <strong>{note.sender}</strong>.</p>
                        <p>
                          ➡️ Updates:{" "}
                          {Object.entries(note.updates).map(([field, value], i) => (
                            <span key={i}>
                    ...
                    {note.type === 'task_reassigned' && (
                      <>
                        <p>🔁 You have been assigned a task: "<strong>{note.message}</strong>"</p>
                        <p>➡️ Re-assigned to you by <strong>{note.sender}</strong>.</p>
                      </>
                    )}
                  </div>
